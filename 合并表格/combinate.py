...
def sele_xlsx():
    ...
    def screen_work():
        ...
        # 读取第一个excel文件，拿取表头
        first_wp = openpyxl.load_workbook(file_list_path + '/' + main_list[0], data_only=True)
        first_sheet = first_wp.active
        for first_col in range(1, first_sheet.max_column + 1):
            #print(first_col)
            new_first_cell = new_sheet.cell(row=1, column=first_col)
            old_first_cell = first_sheet.cell(row=1, column=first_col)
            if old_first_cell.value != None:
                new_first_cell.value = copy(old_first_cell.value)
            if old_first_cell._style is not None:
                new_first_cell.font = copy(old_first_cell.font)
                new_first_cell.fill = copy(old_first_cell.fill)
                new_first_cell.border = copy(old_first_cell.border)
                new_first_cell.number_format = copy(old_first_cell.number_format)
                new_first_cell.protection = copy(old_first_cell.protection)
                new_first_cell.alignment = copy(old_first_cell.alignment)
        ...
        for i, file in enumerate(main_list):
            ...
            # print(file_path)每个excel文件的单独路径
                ...
                tk.messagebox.showinfo(title='提示', message=file+'文件错误')
            # 根据表里所有的sheet进行操作
            for old_sheet in wp.worksheets:
                # 获取原sheet的最大行数和列数
                ...
                for row in range(1, max_row + 1):
                    ...
                    for col in range(1, max_col + 1):
                        if row !=1:
                            new_cell=new_sheet.cell(row=row_cal, column=col)
                            old_cell=old_sheet.cell(row=row, column=col)
                            if old_cell.value != None:
                                new_cell.value = copy(old_cell.value)
                            if old_cell._style is not None:
                ...
                for row_r in range(1, max_row + 1):
                    new_sheet.row_dimensions[row_r].height = copy(old_sheet.row_dimensions[row_r].height)
                    # print(sheet.row_dimensions[row_r].height)
                for col_c in range(1, max_col + 1):
                    # print(get_column_letter(col_c))
                    new_sheet.column_dimensions[get_column_letter(col_c)].width = copy(
                        old_sheet.column_dimensions[
                            get_column_letter(col_c)].width)

                # 整合合并单元格
                merge_list = list(old_sheet.merged_cells)
                # print(merge_list)
                if len(merge_list) > 0:
